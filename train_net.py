...
from fsdet.engine.hooks import HookBase # import hook from fsdet's library (a copy of Detectron2)

class EarlyStoppingHook(HookBase):
    def __init__(self, trainer, cfg, patience=15):
        self.trainer = trainer
        self.cfg = cfg
        self.dataset_name = cfg.DATASETS.TEST
        self.patience = patience
        self.counter = 0
        self.output_folder = cfg.OUTPUT_DIR

    def after_step(self):
        if self.trainer is None:
            raise ValueError("Trainer has not been set for the hook")
        # do something here, for example. COCO evaluator
        # print(f"Running validation check at iter {self.trainer.iter}...")
        else:
            dataset_name = self.cfg.DATASETS.TEST
            # print(dataset_name)
            # do something to deal with early stopping here
            # evaluate and monitor the mAP e.g. after every 500 steps and record the result
            pass
...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type == "lvis":
            return LVISEvaluator(dataset_name, cfg, False, output_folder)
    ...
    def build_hooks(self):
        ...
        hooks.append(EarlyStoppingHook(self, self.cfg))
        return hooks
    """
