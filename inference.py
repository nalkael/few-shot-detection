...
cfg.MODEL.WEIGHTS = os.path.join("checkpoints/coco/faster_rcnn/faster_rcnn_R_50_FPN_ft_all_30shot/model_final.pth")
...
def inference_image(image_path: str, cfg):
...
def inference_images(image_dir: str, cfg):
    # define the folder
    ...
    for image_path in select_images:
        inference_image(image_path, cfg)
        # key = cv2.waitKey(0)  # Wait for a key press to close the image window

if __name__ == "__main__":
    inference_images("datasets/coco/640x640_coco/valid", cfg)