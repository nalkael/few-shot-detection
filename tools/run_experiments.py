...
def parse_args():
    ...
    parser.add_argument(
        "--suffix", type=str, default="", help="Suffix of path"
    )
    parser.add_argument(
    ...
    # Model
    parser.add_argument(
        "--fc", action="store_true", help="Model uses FC instead of cosine"
    )
    parser.add_argument(
        "--two-stage", action="store_true", help="Two-stage fine-tuning"
    )
    parser.add_argument(
        "--novel-finetune",
        action="store_true",
    ...
    parser.add_argument(
        "--unfreeze", action="store_true", help="Unfreeze feature extractor"
    )
    # PASCAL arguments
    parser.add_argument(
        "--split", "-s", type=int, default=1, help="Data split"
    )
    # COCO arguments
...
def run_exp(cfg, configs):
    ...
    res_path = os.path.join(
        args.root, output_dir, "inference", "res_final.json"
    )
...
def get_config(seed, shot):
    ...
    if args.coco:
        ...
        assert (
            not args.two_stage
        ), "Only supports random weights for PASCAL now"
    ...
    temp = os.path.join(
        temp_split,
        "faster_rcnn_R_101_FPN_ft{}_{}_1shot{}".format(
    ...
    if args.coco and not args.novel_finetune:
        ...
        if not os.path.exists(ckpt_path):
            ...
            if not os.path.exists(src2):
                print(
                    "Novel weights do not exist. Please run with the "
                    + "--novel-finetune flag first."
                )
                assert False
            combine_cmd = (
                "python tools/ckpt_surgery.py --coco --method "
    ...
    configs["DATASETS"]["TRAIN"] = (
        configs["DATASETS"]["TRAIN"][0].replace("1shot", str(shot) + "shot")
...
def main(args):
    for shot in args.shots:
        for seed in range(args.seeds[0], args.seeds[1]):
            print(
                "Split: {}, Seed: {}, Shot: {}".format(args.split, seed, shot)
            )
